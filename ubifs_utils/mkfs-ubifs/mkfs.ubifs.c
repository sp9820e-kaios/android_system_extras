...
#define PROGRAM_NAME "mkfs_ubifs"
...
#include "../libselinux/src/selinux/selinux.h"
#include "../libselinux/src/selinux/label.h"
...
static int mount_len;
static char *change_node;
#endif
static char *root;
static int root_len;
...
static const char *optstring = "d:r:m:o:D:yh?vVe:c:g:f:Fp:k:x:X:j:R:l:j:UQqS:a:";
...
"-a, --mount-point        android mountpoint\n"
...
#ifdef ANDROID
		case 'a':
			mount_len = strlen(optarg);
			mount_point = malloc(mount_len + 1);
			if (!mount_point)
				return err_msg("cannot allocate memory");
			/*
			* The further code expects '/' at the end of the root
...
//	   return add_inode(st, inum, flags);
   ...
   if ( context ) {
	...
	asprintf(&change_node, "/%s%s", mount_point, sepath);
	dbg_msg(2, "---filecontxt %s", change_node);
      ...
      if (selabel_lookup(sehnd, &secontext,
         ...
         printf("missing context: %s\t%s\t%d\n", secontext, change_node, st->st_mode);
      }
      free(sepath);
...
#endif
		asprintf(&change_node, "%s%s", mount_point, name + root_len - 1);
		dbg_msg(2, "---3 %s", change_node);
#if 1
		fs_config(change_node, (int)(S_ISDIR(dent_st.st_mode)),NULL,&uid, &gid, &mode,&capabilities);
#else
		fs_config(change_node, (int)(S_ISDIR(dent_st.st_mode)),NULL,,&uid, &gid, &mode);
#endif
		dbg_msg(2, "---4 %o %o", dent_st.st_mode,mode);
		dbg_msg(2, "---5 %x %x", dent_st.st_uid,uid);
		dbg_msg(2, "---6 %x %x", dent_st.st_gid,gid);
		dent_st.st_uid = uid;
		dent_st.st_gid = gid;
		dent_st.st_mode = (dent_st.st_mode & (~07777)) | mode;
		dbg_msg(2, "---7 %o", dent_st.st_mode);
#endif
...
static int write_data(void)
{
	...
	if (root) {
		...
		if (err)
			...
			uint64_t capabilities;
#endif
			asprintf(&change_node, "%s", mount_point);
			dbg_msg(2, "---root %s", change_node);
#if 1
			fs_config(change_node, (int)(S_ISDIR(root_st.st_mode)),NULL,&uid, &gid, &mode,&capabilities);
#else
			fs_config(change_node, (int)(S_ISDIR(root_st.st_mode)),NULL,&uid, &gid, &mode);
#endif
			dbg_msg(2, "---root %o %o", root_st.st_mode,mode);
			dbg_msg(2, "---root %x %x", root_st.st_uid,uid);
			dbg_msg(2, "---root %x %x", root_st.st_gid,gid);
			root_st.st_uid = uid;
			root_st.st_gid = gid;
			root_st.st_mode = (root_st.st_mode & (~07777)) | mode;
			dbg_msg(2, "---root %o", root_st.st_mode);
#endif
	} else {
